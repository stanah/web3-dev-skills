...
This command performs 6 phases:
...
## Phase 4: Select Output Language

Ask the user which language the SPECA pipeline output should be written in. This affects all human-readable text across the entire pipeline (intermediate JSON files, reports, and console summaries).

**Prompt the user:**
> What language should the SPECA output be written in? This applies to all human-readable text in the pipeline (requirements, checklist properties, findings, reports, and summaries).
>
> Examples: `en` (English), `ja` (Japanese), `zh` (Chinese), `ko` (Korean), `es` (Spanish)
>
> Default: `en` (English). Press enter to use the default.

**After receiving input:**
1. If the user provides no input (empty), use `"en"` as the default.
2. Otherwise, store the provided language code (lowercase, trimmed) as `language`.
3. This setting affects all human-readable text generated by SPECA commands. Machine-readable identifiers (requirement IDs, checklist IDs, finding IDs, severity level names), Solidity code, file paths, and SARIF output always remain in English.

Store the value as a string in `language`.
