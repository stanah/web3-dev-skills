...
12. Parse the JSON and extract the `source_paths` array. Also extract the `language` field. If `language` is not present, default to `"en"` (English). This setting determines the language for human-readable text generated by this command (test comments, `interpretation` strings in test-results.json, console summary output). Solidity code, identifiers, and test function names always remain in English.
...
Print a summary to the user. **All console output text (headers, labels, next steps) MUST be written in the language specified by `language`.** If `language` is `"en"` or not set, use the English text shown below as-is.
