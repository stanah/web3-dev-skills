...
4. Extract the `language` field from the config. If `language` is not present, default to `"en"` (English). This setting determines the language for all human-readable text generated by this command (`evidence` strings, `note` strings for unmapped requirements, console summary output). Identifiers (requirement IDs, file paths, confidence scores) always remain in English.
...
After writing the file, print a summary to the user. **All console output text (headers, labels, warnings, next steps) MUST be written in the language specified by `language`.** If `language` is `"en"` or not set, use the English text shown below as-is.
