...
4. Extract the `language` field from the config. If `language` is not present, default to `"en"` (English). This setting determines the language for all human-readable text generated by this command (auto-generated requirement text from YAML specs, console summary output). Identifiers (requirement IDs, modal verbs, type labels) and text extracted verbatim from spec files always remain in their original form.
5. Validate that each file listed in `spec_paths` exists by reading it. If any file is missing, report which file(s) are missing and stop.
