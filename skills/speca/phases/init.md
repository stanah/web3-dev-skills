...
```bash
echo '<config JSON>' | node $SPECA_DIR/scripts/speca-cli.mjs config --action init
```
...
- Before writing config, run `node $SPECA_DIR/scripts/speca-cli.mjs config --action validate`. If exit code is 0 (config exists and is valid), ask the user: "A SPECA config already exists. Overwrite? (yes/no)". If exit code is 1 with `FILE_NOT_FOUND`, proceed normally.
